...
KMP попередньо обробляє шуканий підрядок, створюючи LPS-масив (*Longest Prefix Suffix*). Це дозволяє при неспівпадінні знати, на скільки символів можна безпечно зсунутися, не повертаючись назад у тексті.
...
*Примітка: Точні цифри залежать від процесора при запуску скрипта `run_benchmark.py`. Нижче наведено аналіз на основі типової поведінки цих алгоритмів на текстах такого обсягу.*
